...
class AuditTest {
    ...
    @Test fun persistsAndCaps() {
        ...
        a.add(Audit.Kind.NAMES, "творог", "твор+ог", "снова")
        a.flush()
        val b = Audit(tmp.root)
        val e = b.entries(Audit.Kind.NAMES).single()
        assertEquals("тв+орог", e.variant); assertEquals(2, e.count); assertEquals("ел творог с молоком", e.context)
        for (i in 0 until Audit.MAX + 5) b.add(Audit.Kind.NAMES, "слово$i", "сл+ово$i", "")
        assertEquals(Audit.MAX, b.entries(Audit.Kind.NAMES).size)
        assertNull(b.entries(Audit.Kind.NAMES).firstOrNull { it.word == "творог" }) // самое старое вытеснено
        b.remove(Audit.Kind.NAMES, "слово${Audit.MAX + 4}")
    ...
    @Test fun textAndLoadRoundTrip() {
        val a = Audit(tmp.root)
        a.add(Audit.Kind.NAMES, "творог", "тв+орог", "ел творог"); a.hide(Audit.Kind.NAMES, "творог", true)
        val text = a.text(Audit.Kind.NAMES)
        assertEquals("творог\tтв+орог\t1\tел творог\th\n", text)
        val b = Audit(tmp.root); b.clear(Audit.Kind.NAMES, hidden = true)
        b.load(Audit.Kind.NAMES, text)
        assertEquals(listOf("творог"), b.entries(Audit.Kind.NAMES, hidden = true).map { it.word })
    }
}
