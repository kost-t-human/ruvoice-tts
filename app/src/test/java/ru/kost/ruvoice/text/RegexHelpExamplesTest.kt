...
class RegexHelpExamplesTest {
    ...
    @Test fun examplesFromHelp() {
        ...
        assertEquals("{prosody:80:90} Не оборачивайся.", run("""\(шёпотом\)""", "{prosody:80:90}", "(шёпотом) Не оборачивайся."))
        assertEquals("это *именно* он", run("""\b(именно|только)\b""", "*$1*", "это именно он"))
