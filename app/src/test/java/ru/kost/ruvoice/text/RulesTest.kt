...
class RulesTest {
    ...
    @Test fun timeWithFractionalSecondsIsNotDate() {
        // «07.2» после «06:58:» — доли секунды, а не «седьмое февраля».
        assertEquals("шесть часов пятьдесят восемь минут семь целых две десятых секунды GMT", n("06:58:07.2 GMT"))
        assertEquals("в двенадцать часов тридцать минут пять целых двадцать пять сотых секунды", n("в 12:30:05.25"))
    }

    @Test fun yearsWithG() {
        // «г.» в конце текста — ещё и точка предложения, она остаётся.
    ...
    // Граммы против годов (разбор по примерам смежной сессии). Признаки граммов: слово-продукт
    // следом, слово веса/тары перед, «=» следом, число больше 2100, одно-двузначное без «н. э.»;
    // признаки года: предлог года + 3–4 цифры (точка не обязательна после «в»/«к»), «н. э.», 4 цифры + «г.».
    @Test fun gramsVsYears() {
        for ((inp, exp) in listOf(
            "500 г муки" to "пятьсот граммов муки", "500г муки" to "пятьсот граммов муки", "1 г соли" to "один грамм соли",
            "21 г белка" to "двадцать один грамм белка", "2 г дрожжей" to "два грамма дрожжей",
            "3 г, 4 г, 22 г" to "три грамма, четыре грамма, двадцать два грамма",
            "0,5 г ванилина" to "ноль целых пять десятых грамма ванилина", "Добавьте 200 г." to "Добавьте двести граммов.",
            "1 кг 500 г" to "один килограмм пятьсот граммов", "5 г 200 мг" to "пять граммов двести миллиграммов",
            "200 гр. муки" to "двести граммов муки", "500 Г" to "пятьсот граммов", "100 г/л" to "сто граммов на литр",
            "около 200 г" to "около двухсот граммов", "из 300 г муки" to "из трёхсот граммов муки", "до 100 г" to "до ста граммов",
            "не менее 10 г" to "не менее десяти граммов", "к 5 г добавить" to "к пяти граммам добавить", "по 50 г" to "по пятьдесят граммов",
            "в 100 г продукта содержится" to "в ста граммах продукта содержится",
            "в 500 г" to "в пятисотом году", "в 862 г был призван Рюрик" to "в восемьсот шестьдесят втором году был призван Рюрик",
            "в 1147 г впервые упомянута Москва" to "в тысяча сто сорок седьмом году впервые упомянута Москва",
            "в 500 г муки содержится 1700 ккал" to "в пятистах граммах муки содержится одна тысяча семьсот килокалорий",
            "весом в 300 г" to "весом в триста граммов", "упаковка в 250 г" to "упаковка в двести пятьдесят граммов",
            "с 862 г по 879 г" to "с восемьсот шестьдесят второго по восемьсот семьдесят девятый год",
            "до 500 г" to "до пятисот граммов",
            "1500 г фарша" to "одна тысяча пятьсот граммов фарша", "2500 г" to "две тысячи пятьсот граммов",
            "1000 г = 1 кг" to "одна тысяча граммов равно один килограмм",
            "к 1900 г." to "к тысяча девятисотому году.", "в конце 1990 г." to "в конце тысяча девятьсот девяностого года.",
            "в течение 2020 г." to "в течение две тысячи двадцатого года.", "летом 1941 г." to "летом тысяча девятьсот сорок первого года.",
            "в мае 1945 г." to "в мае тысяча девятьсот сорок пятого года.", "декабрь 1825 г." to "декабрь тысяча восемьсот двадцать пятого года.",
            "зима 1941 г." to "зима тысяча девятьсот сорок первого года.",
            "с 1991 г. по 2000 г." to "с тысяча девятьсот девяносто первого по двухтысячный год",
            "между 1990 и 2000 гг." to "между тысяча девятьсот девяностым и двухтысячным годами",
            "данные за 2020 г. показали" to "данные за две тысячи двадцатый год показали",
            "(1799 г.)" to "(тысяча семьсот девяносто девятый год)", "2050 г" to "две тысячи пятидесятый год",
            "в 79 г. н. э." to "в семьдесят девятом году нашей эры", "в 44 г. до н. э. убит Цезарь" to "в сорок четвёртом году до нашей эры убит Цезарь",
            "с 27 г. до н. э. по 14 г. н. э." to "с двадцать седьмого года до нашей эры по четырнадцатый год нашей эры",
            "Родился в 1828 г. Умер в 1910 г." to "Родился в тысяча восемьсот двадцать восьмом году. Умер в тысяча девятьсот десятом году.",
            "Это было в 1991 г. СССР распался." to "Это было в тысяча девятьсот девяносто первом году. СССР распался.",
            "Л. Н. Толстой родился в 1828 г. в Ясной Поляне." to "Л. Н. Толстой родился в тысяча восемьсот двадцать восьмом году в Ясной Поляне.",
            "ул. Ленина, д. 5, г. Москва" to "улица Ленина, д. пять, город Москва", "2 г. р." to "второго года рождения",
        )) assertEquals(inp, exp, n(inp))
    }
    ...
    @Test fun abbreviations() {
        ...
        // падеж по предлогу перед номером
        assertEquals("до страницы пять", n("до стр. 5"))
        assertEquals("со страницы пять", n("с стр. 5"))
        assertEquals("из таблицы два", n("из табл. 2"))
        assertEquals("к рисунку три", n("к рис. 3"))
        assertEquals("под рисунком три", n("под рис. 3"))
        assertEquals("в статье сто пять", n("в ст. 105"))
        assertEquals("по статье сто пять", n("по ст. 105"))
        assertEquals("согласно пункту три", n("согласно п. 3"))
        assertEquals("в пункте пять", n("в п.5"))
        assertEquals("из главы второй", n("из гл. 2"))
        // без номера — только «на/в/о/при» и «стр./гл./табл./рис.»; «из рис.» — крупа
        assertEquals("в таблице", n("в табл."))
        assertEquals("каша из рис.", n("каша из рис."))
    ...
    @Test fun currencySignAfterNumberBeforeCents() {
        // «$» после числа относится к нему, а не к центам: было «один девяносто девять долларов центов»
        assertEquals("один доллар девяносто девять центов", n("1 $ 99 центов"))
        assertEquals("два доллара пятьдесят центов", n("2 $ 50 ¢"))
        assertEquals("один евро девяносто девять центов", n("1 € 99 центов"))
        assertEquals("от пяти долларов до десяти долларов", n("от $5 до $10"))
    }
    ...
    // «день месяц гггг г.» — день порядковый, с годом — в родительном (обстоятельство времени),
    // год через «г.» — год, не граммы.
    @Test fun dayMonthYearWithG() {
        assertEquals("двадцатого июня две тысячи пятидесятого года.", n("20 июня 2050 г."))
        ...
        assertEquals("двадцатого июня две тысячи пятидесятого года", n("20 июня 2050 года"))
