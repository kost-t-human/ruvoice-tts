...
/** Символы входа модели: общее у штатной v5_5_ru (silero_ru.json) и паков голосов (pack.json). */
class Symbols(val symbols: String, val symbolToId: Map<Char, Int>, val sos: Char, val eos: Char, val alphabet: Set<Char>) {
    /** Символы, допустимые во входе модели: без трёх служебных в начале строки — ровно фильтр
     * `symbols[3:]` из prepare_text_input пакета Silero (у v5_5_ru это `_~|`, у cis-пака `|!'`,
     * так что там выпадают «!» и апостроф). */
