...
# «все» + сравнительная степень → «всё» («всё сильнее», «всё больше»): по книгам с честной «ё» 97,7 % из 12 тыс.;
# «ранее/заранее» — нет («все ранее начатые»), «более-менее» — тоже (дефис), «ее» — это «её»
VSE_CMP = set('больше меньше лучше хуже чаще реже дальше ближе выше ниже громче тише глубже короче дороже дешевле легче проще '
              'строже моложе старше позже раньше жарче ярче крепче слаще жестче резче толще тоньше шире круче гуще мягче'.split())
VSE_CMP_NOT = {'ее', 'ранее', 'заранее'}


def vse_yo(nxt, after=''):
    """«все» перед сравнительной степенью → «вс+ё»; after — текст сразу после nxt (дефис — «более-менее»); None — молчим."""
    nxt = nxt.replace('+', '').replace('ё', 'е')
    if nxt in VSE_CMP_NOT or after.startswith('-'): return None
    return 'вс+ё' if nxt.endswith('ее') or nxt in VSE_CMP else None
